...
export function readScenario() {
  ...
  group('게시글 상세 조회', () => {
    ...
    const ok = check(res, {
      '상세 조회 성공': (r) => r.status === 200 || r.status === 404,
    });
    errorRate.add(!ok);
  });
  ...
  group('댓글 목록 조회', () => {
    ...
    const ok = check(res, {
      '댓글 조회 성공': (r) => r.status === 200 || r.status === 404,
    });
    errorRate.add(!ok);
  });
...
export function writeScenario() {
  ...
  group('게시글 작성', () => {
    ...
    if (res.status === 201) {
      ...
      const commentRes = http.post(
        `${API}/posts/${post.postId}/comments?testUserId=${userId}`,
        JSON.stringify({ content: '테스트 댓글입니다.', parentCommentId: null }),
        { headers }
      );
      check(commentRes, { '댓글 작성 201': (r) => r.status === 201 });
    }
  });
  ...
  group('좋아요 토글', () => {
    const postId = randomPostId();
    const res = http.post(`${API}/posts/${postId}/likes?testUserId=${userId}`);
